...
> 📐 Редактируемый исходник: **`version-a-sectioned.drawio`** (страницы «Компоненты», «Состояния»).
> 📱 Поведение на мобильных (спека для UX/UI): [version-a-mobile.md](./version-a-mobile.md).
> Домен-модель и обработка ошибок — `domain-model.drawio`.
