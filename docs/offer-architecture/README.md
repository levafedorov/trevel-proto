...
Каждая диаграмма поставляется в **двух форматах**:

- **`.drawio`** — редактируемый исходник (diagrams.net / draw.io). Это основной deliverable
  для UX/UI: открывается на [app.diagrams.net](https://app.diagrams.net) (File → Open) или
  в расширении **Draw.io Integration** для VS Code. Файлы многостраничные — вкладки страниц
  снизу (Компоненты · Состояния · и т.д.).
- **`.md`** — то же самое в виде **Mermaid** (для быстрого просмотра прямо на GitHub) +
  текстовое описание, плюсы/минусы и готовность к CMS.

### Файлы

| Файл | Что внутри |
|---|---|
| `domain-model.drawio` | Общая доменная модель (class diagram) — одна для всех версий |
| `version-a-sectioned.drawio` | Версия A: страницы «Компоненты», «Состояния» |
| `version-b-blocks.drawio` | Версия B: «Компоненты», «Реестр блоков», «Состояния» |
| `version-c-tabbed.drawio` | Версия C: «Компоненты», «Модель навигации», «Состояния» |
| `README.md` + `version-*.md` | Описания (Mermaid + текст) |

## Как этим пользоваться

1. Прочитайте этот файл — здесь **общая для всех версий** доменная модель данных
   (`domain-model.drawio`), каталог edge-кейсов и поток данных из CMS.
2. Откройте три версии и сравните их подход к **структуре страницы**:
   - [Версия A — Секционная](./version-a-sectioned.md) · `version-a-sectioned.drawio` — простая, линейная, быстрый старт.
   - [Версия B — Блочная / CMS-native](./version-b-blocks.md) · `version-b-blocks.drawio` — максимально гибкая, редактор-конструктор.
   - [Версия C — Вкладки / прогрессивное раскрытие](./version-c-tabbed.md) · `version-c-tabbed.drawio` — лучший UX для длинного контента.
3. Выберите **одну** и отправьте её `.drawio` в UX/UI. Доменная модель и обработка ошибок
